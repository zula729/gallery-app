...
class FirebaseClient:
    ...
    def push_text(self, files: list[Path], doc_processor):
        for file_path in files:
            ...
            self._merge_and_push(folder_id, {"text": text})
    ...
    def push_keywords(self, files: list[Path], extractor, doc_processor,keyword_filter):
        for file_path in files:
            if PathParser.is_macos_artifact(file_path):
                continue
    ...
    def push_keywords_from_json(self, json_path: str):
        data = JsonYamlManager.load_json(json_path)
        for folder_id, keywords in data.items():
            self._merge_and_push(folder_id, {"keywords": keywords})
    ...
    def find_missing_folders(self, root_dir: Path):
        ...
        if missing:
            ...
            for fid, name in sorted(missing.items()):
                print(f"{fid}->{name}")
        self._report_missing(local_ids, db_ids, missing)
        return missing

...
class KeywordExtractor:
    ...
    def extract_technology(self, file_path: Path, doc_processor: DocumentProcessor) -> list[str]:
        TECH_TERMS = ProjectUtils.load_categories("tech_terms.yaml") 
...
class FirebaseClient:
    ...
    def upload_tags(self, uco: str, extractor: KeywordExtractor, yaml_path: str):
        tags = ProjectUtils.load_categories(yaml_path)
        enter = self.fetch_all().get(uco)
        if enter is None:
            return set()
        keywords = self.fetch_all()[uco].get('keywords', [])
        found_tags = set()
